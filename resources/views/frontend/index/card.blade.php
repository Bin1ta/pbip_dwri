@php use App\Helpers\Helpers;use Illuminate\Support\Str; @endphp
<div class="row">
    @foreach($categories as $category)
        <div class="col-lg-4 mt-2">
            <div class="card-01">
                <h6 class="heading-line mb-3 text-center pt-1"
                    style="background-color: {{ $colors->nav ?? '' }}; color: white">
                    ...
                    @foreach($category->documentCategories as $subcategory)
                        <li class="nav-item" role="presentation">
                            <button class="nav-link {{$loop->first ? 'active': ''}}"
                                    id="{{ Str::slug($subcategory->title,'-') }}-tab"
                                    data-bs-toggle="tab" data-bs-target="#{{ Str::slug($subcategory->title,'-') }}"
                                    type="button" role="tab" aria-controls="{{ Str::slug($subcategory->title,'-') }}"
                ...
                <div class="tab-content card-content" id="myTabContent">
                    @forelse($category->documentCategories as $subcategoryData)
                        <div class="tab-pane fade {{$loop->first ? 'show active': ''}}"
                             id="{{ Str::slug($subcategoryData->title,'-') }}"
                             role="tabpanel" aria-labelledby="{{ Str::slug($subcategoryData->title,'-') }}-tab">
                                ...
                                @foreach($subcategoryData->documents->take(3) as $document)
                                    <a href="{{route('documentDetail',[$document->slug,'language'=>$language])}}"
                                       class="card-01 mb-2 border">
                                        ...
                                        <p class="mt-2">
                                            @if(request()->language=='en')
                                                {{Helpers::getEnglishNumber($document->published_date)}}
                                                | {{$document -> publisher_en}}
                                            @else
                                                {{Helpers::getNepaliNumber($document->published_date)}}
                                                | {{$document -> publisher}}
                                            @endif
                                        </p>
                                    </a>
                                ...
                                <a href="{{route('documentDetail',[$document->slug,'language'=>$language])}}"
                                   class="card-01 mb-2 border">
                                    @if(request()->language=='en')
                                        ...
                                        <p class="mt-2">{{$document -> published_date}}
                                            | {{$document -> publisher_en}}</p>
                                    @else
